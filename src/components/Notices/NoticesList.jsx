...
import { selectFavoritesIds, selectLastPage, selectNotices, selectSexValue } from "../../redux/notices/noticesSelectors";
import { useEffect, useState} from "react";
import { addFavoriteNotice, getAllNotices, getNotices, removeFavoriteNotice } from "../../redux/notices/noticesOperations";
import { getCurrentUser } from "../../redux/auth/authOperations";
import { selectUser } from "../../redux/auth/authSelectors";

// eslint-disable-next-line react/prop-types
const NoticesList = ({page, setPage, searchWord, category, type}) => {
  ...
  useEffect(() => {
    setPage(1)
  },[searchWord, category, setPage, type, gender])

  useEffect(() => {
    dispatch(getNotices({ page, keyword: searchWord, category, species: type }))
    if (gender !== "all") {
      dispatch(getAllNotices({ page, keyword: searchWord, category, species: type, limit: lastPage * 6}))
    }
  }, [dispatch, page, searchWord, category, type, gender, lastPage]);
  ...
  return (
    (gender === "all" && notices.length > 0 || gender !== "all" && notices.length > 0) ?
    (<ul className="my-10 xl:px-8 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 xl:gap-x-8 xl:gap-y-10 gap-5">
      ...
      : (<p>There are no items for your request. Please, try change your search params</p>)
  )
}
